 ...
 * Root endpoint.
 *
 * This service is the Shave API. The previous implementation had no
 * GET / route, so opening the Render URL reached the catch-all 404 handler.
 *
 * This endpoint does not fabricate financial data. It only confirms that
...
app.get('/db-check', async (req, res) => {
  try {
    const result = await pool.query(
      'select now() as db_time, count(*) as user_count from users'
    );
...
app.post('/plaid/create-update-link-token', requireAuth, async (req, res) => {
  try {
    ...
    const accessToken = decrypt(
      itemRow.rows[0].plaid_access_token_encrypted
    );
...
app.post('/plaid/exchange-public-token', requireAuth, async (req, res) => {
  try {
    ...
    try {
      ...
      immediateSyncResult =
        await syncOneItem(freshItem.rows[0]);
    } catch (syncErr) {
      console.error(
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      const result =
        await syncOneItem(itemRow.rows[0]);
