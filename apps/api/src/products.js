...
async function syncLiabilities(accessToken, acctMap) {
  ...
  try {
    ...
    // Not every linked institution/account type returns liabilities data —
    // Plaid returns an error for Items with nothing to report here. Treat as
    // "nothing to sync," not a failure of the whole sync run.
    return;
  }
  if (!data) return;
    ...
    await pool.query(
      `insert into liabilities_credit
        (account_id, aprs, is_overdue, last_payment_amount, last_payment_date,
         last_statement_balance, minimum_payment_amount, next_payment_due_date, updated_at)
      ...
      [
        ...
        credit.last_statement_balance, credit.minimum_payment_amount, credit.next_payment_due_date,
      ]
    );
  }
  ...
  for (const student of data.student || []) {
    ...
    await pool.query(
      `insert into liabilities_student
        (account_id, loan_name, servicer_address, interest_rate_percentage,
         minimum_payment_amount, next_payment_due_date, origination_principal_amount,
         outstanding_interest_amount, updated_at)
       values ($1,$2,$3,$4,$5,$6,$7,$8, now())
       on conflict (account_id) do update set
         loan_name = excluded.loan_name, servicer_address = excluded.servicer_address,
      ...
      [
        accountId, student.loan_name, JSON.stringify(student.servicer_address || {}),
        student.interest_rate_percentage, student.minimum_payment_amount,
        student.next_payment_due_date, student.origination_principal_amount,
